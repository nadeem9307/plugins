...
/**
 * Chotu_Product_Tag_Admin
 */
class Chotu_Rootshop_Cat_Admin{	
	/**
	 * rootshop_cat_id
	 *
	 * @var mixed
	...
	public function __construct() {
		 ...
		 */
		add_action( 'rootshop_cat_add_form_fields',function(){
		 ...
		 */
		add_action( 'created_rootshop_cat',function($term_id, $tt_id){
		 ...
		 */
		add_filter( 'manage_edit-rootshop_cat_columns',function ($columns){
			$res = array_slice($columns, 0, 1, true) +
			array( 'rootshop_cat_image' => __( 'Image', 'chotu' ) ) +
		 ...
		 */
		add_filter( 'manage_rootshop_cat_custom_column', function($string, $columns, $term_id){
			if ( $columns == 'rootshop_cat_image' ) {
