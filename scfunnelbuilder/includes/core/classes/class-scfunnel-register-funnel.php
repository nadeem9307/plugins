 ...
 * Register funnel
 * 
 * @package SCFunnelbuilder\CPT
 */
namespace SCFunnelbuilder\CPT;
...
class ScFunnel_CPT
{
    ...
    {
        $labels = [
            'name'                  => _x('Funnels', 'Funnel general name', 'scfunnelbuilder'),
            'singular_name'         => _x('Funnel', 'Funnel singular name', 'scfunnelbuilder'),
        ];
        $args = [
            'labels'             => $labels,
            'public'             => true,
            ...
            'hierarchical'       => false,
            'show_in_rest'       => true,
            'supports'           => [ 'title', 'editor', 'elementor', 'revisions', 'thumbnail'],
        ];
        register_post_type(SC_FUNNEL_FUNNELS_POST_TYPE, $args);
    }
    ...
    {
        $permalink_settings = ScFunnel_functions::get_permalink_settings();

        $labels = [
            'name'                  => _x('Steps', 'Steps general name', 'scfunnelbuilder'),
            'singular_name'         => _x('Step', 'Step singular name', 'scfunnelbuilder'),
            'edit_item'             => __( 'Edit Step', 'scfunnelbuilder' ),
        ];
        $args = [
            'labels'              => $labels,
            'public'              => true,
        ...
        //         case 'funnel-step':
        //             $structure = '/scfunnels/%funnel_name%/scfunnel_steps';
        //             break;
        //         case 'funnel':
        //             $structure = '/scfunnels/%funnel_name%';
        //             break;
        //         case 'step':
        //             $structure = '%funnel_name%/scfunnel_steps';
        //             break;
        //     }
        ...
        //     ];
        // }
        register_post_type(SC_FUNNEL_STEPS_POST_TYPE, $args);
    }
    ...
    */
    ...
    public function rewrite_step_rule()
    {
        ...
        if (isset($permalink_settings['structure'])) {
            switch ($permalink_settings['structure']) {
                case 'funnel-step':
                    add_rewrite_rule('^' . $permalink_settings['funnel_base'] . '/([^/]*)/' . $permalink_settings['step_base'] . '/([^\/]*)/?', 'index.php?scfunnel_steps=$matches[2]', 'top');
                    break;
                case 'funnel':
                    add_rewrite_rule('^' . $permalink_settings['funnel_base'] . '/([^/]*)/([^/]*)/?', 'index.php?scfunnel_steps=$matches[2]', 'top');
                    break;
                case 'step':
                    add_rewrite_rule('([^/]*)/' . $permalink_settings['step_base'] . '/([^\/]*)/?', 'index.php?scfunnel_steps=$matches[2]', 'top');
                    break;
                default:
                    break;
            }
    ...
    public function step_post_type_permalinks( $post_link, $post, $leavename )
    {
        ...
        if (isset($post->post_type) && SC_FUNNEL_STEPS_POST_TYPE == $post->post_type) {
            ...
            if (isset($permalink_settings['structure']) && ! empty($permalink_settings['structure'])) {
                if($permalink_settings['structure'] !== 'default') {
                    $sep       = '/';
                    $post_link = preg_replace('/\/scfunnels\//', '/'.$permalink_settings['funnel_base'].'/', $post_link);
                    $search    = [ $sep . '%funnel_name%', $sep . 'scfunnel_steps' ];
    ...
    public function add_steps_cpt_to_main_query( $query ) {
        ...
        if ( isset( $query->query_vars['post_type'] ) && is_array( $query->query_vars['post_type'] ) ) {
            ...
            $post_types[] = SC_FUNNEL_STEPS_POST_TYPE;
            ...
            $query->set( 'post_type', array( 'post', 'page', SC_FUNNEL_STEPS_POST_TYPE ) );
        }
        return $query;
    }
    ...
    public function print_admin_js_template() {
        ...
        <script id="scfunnel-gutenberg-button-switch-mode" type="text/html">
            <div id="scfunnel-switch-mode">
                <a href="<?php echo esc_url($funnel_window_url); ?>" id="scfunnel-switch-mode-button" type="button" class="button button-primary button-large">
                    <span class="scfunnel-switch-mode-on"><?php echo __( '&#8592; Back to Funnel Window', 'scfunnelbuilder' ); ?></span>
                </a>
            </div>
        </script>
    <?php
